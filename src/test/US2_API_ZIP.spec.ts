...
describe('US_02 | Upload zip function', () => {
  ...
  it('TC01: Allow user to upload a zip file containing multiple valid image files', async () => {
    const response = await zipStep.uploadAnZip("data\\zipfile\\3validzip.zip")
    ...
    const responseLink = await zipStep.getLinkInZip(await imgLinks[index])
  ...
  it('TC02: Do not allow an Anonymous user to upload an empty zip file', async () => {
    const response = await zipStep.uploadAnZip("data\\zipfile\\empty.zip")
  ...
  it('TC03: Do not allow an Anonymous user to upload invalid zip file', async () => {
    const response = await zipStep.uploadAnZip("data\\zipfile\\samplePDF.pdf")
